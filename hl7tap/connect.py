"""Native cognis-connect emit for hl7tap — forward findings to any platform.

Maps hl7tap's JSON output to the canonical `Finding` and forwards it via
...
Usage:
    hl7tap ... --format json | hl7tap-emit --to stix
    hl7tap-emit --to slack --url $WEBHOOK --dry-run < findings.json
"""
...
def map_record(rec: dict) -> dict:
    ...
    try:
        out = dict(rec)
        if 'message_type' in rec:
            out['title'] = f"{rec['message_type']} - {rec.get('trigger_event', '')}"
            out['type'] = rec['message_type']
            out['description'] = rec.get('trigger_event', '')
            out['severity'] = 'info'
            if 'segment' in rec and rec['segment'] == 'MSH':
                out['tags'] = ['msh']
            else:
                out['tags'] = []
            if 'pid' in rec:
            ...
            if 'pv1' in rec:
                out['tags'].append('pv1')
            if 'pid' in rec and 'patient_id' in rec['pid']:
                out['indicators'] = {'imo': rec['pid']['patient_id']}
            if 'pv1' in rec and 'patient_location' in rec['pv1']:
                out['indicators'] = {'mmsi': rec['pv1']['patient_location']}
            if 'location' in rec:
                out['indicators'] = {'lat': rec['location'].get('latitude', ''), 'lon': rec['location'].get('longitude', '')}
        if 'cve' in rec:
            out['cve'] = rec['cve']
        if 'ipv4' in rec:
            out['indicators']['ipv4'] = rec['ipv4']
        if 'domain' in rec:
            out['indicators']['domain'] = rec['domain']
        if 'url' in rec:
            out['indicators']['url'] = rec['url']
        if 'sha256' in rec:
            out['indicators']['sha256'] = rec['sha256']
        return out
    except Exception:
        return rec
