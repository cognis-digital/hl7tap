...
**hl7tap** plugs into your stack through [`cognis-connect`](https://github.com/cognis-digital/cognis-connect),
...
Once `hl7tap` emits JSON findings, pipe them straight to a destination — `--dry-run`
previews the exact request without sending:

```bash
hl7tap ... --format json | cognis-connect emit --to stix   # STIX 2.1 bundle
hl7tap ... --format json | cognis-connect emit --to slack --url $URL --token $TOK   # Slack
hl7tap ... --format json | cognis-connect emit --to webhook --url $URL --token $TOK   # generic webhook
```
...
works whatever `hl7tap` outputs:

```python
from cognis_connect import normalize, stix
findings = [normalize(rec, source="hl7tap") for rec in records]   # records = your JSON output
