# Comprehensive installer for cognis-digital/hl7tap (Windows PowerShell).
# Tries: pipx -> uv -> pip (git+https) -> from source.
# hl7tap is source-available and not on PyPI; all paths install from GitHub.
$ErrorActionPreference = "Stop"
$Repo = "hl7tap"
$Url  = "git+https://github.com/cognis-digital/hl7tap.git"
$Git  = "https://github.com/cognis-digital/hl7tap.git"
...
if (Have pipx) {
  ...
  pipx install $Url; if ($LASTEXITCODE -eq 0) { Say "Done. Run: hl7tap"; exit 0 }
}
if (Have uv) {
  Say "Installing with uv..."
  uv tool install $Url; if ($LASTEXITCODE -eq 0) { Say "Done. Run: hl7tap"; exit 0 }
}
if (Have pip) {
  Say "Installing with pip (user site)..."
  pip install --user $Url; if ($LASTEXITCODE -eq 0) { Say "Done. Run: hl7tap"; exit 0 }
}
